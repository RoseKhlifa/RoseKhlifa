_____                _  ___     _ _  __      
 |  __ \              | |/ / |   | (_)/ _|     
 | |__) |___  ___  ___| ' /| |__ | |_| |_ __ _ 
 |  _  // _ \/ __|/ _ \  < | '_ \| | |  _/ _` |
 | | \ \ (_) \__ \  __/ . \| | | | | | || (_| |
 |_|  \_\___/|___/\___|_|\_\_| |_|_|_|_| \__,_|

## 👤 SYSTEM_PROFILE: @RoseKhlifa

> "Code, Automate, Reverse."

### 🛠️ CORE_STACK
* **Primary:** `C++` | `Python` | `Java` | `Go`
* **Exploring:** `Flutter` | `TypeScript`
* **Infrastructure:** Server deployment & environment management (Powered by 1Panel)

### 🔍 RESEARCH_&_INTERESTS
* **Reverse Engineering:** 游戏逆向工程、底层逻辑剖析与引擎机制研究。
* **Automation:** 脚本化、工作流优化与自动化机器人的开发实践。
* **Crypto:** 关注加密货币生态与底层技术架构。
* **Workflow:** 践行 Vibe Coding，利用 AI 辅助重塑开发体验与代码直觉。

### 🔗 LINKS_&_CONTACT
* **Personal Site:** [rosekhlifa.cn](https://rosekhlifa.cn)
* **X:** [@RoseKhlifa888](https://x.com/RoseKhlifa888)